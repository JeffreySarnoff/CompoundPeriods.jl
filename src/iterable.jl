...
Base.iterate(x::CompoundPeriod, state=(length(x), 1)) = 
    state[2] <= state[1] ? (x.periods[state], (state[1], state[2]+1)) : nothing

Base.iterate(x::ReverseCompoundPeriod) = length(x.cperiod.periods)
Base.iterate(x::ReverseCompoundPeriod, state=(length(x), length(x))) =
    state[2] > 0 ? (x.cperiod.periods[state], (state[1], state[2]-1)) : nothing
